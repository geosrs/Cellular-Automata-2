...
def generateCellSpace(rules, state, wrap = True):
    ...
    for cell in range(len(state)):
        for i in range(len(rules)): # i represents the ith rule in rules
            ruleset = {'on': rules[i], 'off': list(set(range(rules[i][0], rules[i][len(rules[i]) - 1])) - set(rules[i]))}
            print(ruleset)
            if wrap == True:
                if all(newstate[cell + index] == 1 for index in ruleset['on']) and all(newstate[cell + index] == 0 for index in ruleset['off']):
                    newstate[cell] = 1
                    # cell += 1
                    #i = 0
                    #j = 0
