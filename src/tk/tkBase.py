...
def createLambda(funct, *variables, **values):
	...
	return lambda: funct(*variables, **values)
...
class tk(object):
	...
	def slideLeft(self, time = None, initialize = False):
		'''Slides a widget to the left (optional time parameter)

		returns None'''
		if initialize:
			self.initializeAnimation(time)
		start = self.widget.winfo_width()
		self.widget.previous_width = start
		def changeWidth(percent):
			self.widget.configure(width = start * percent)
			if percent > 0:
				self.widget.after(self.time, lambda: changeWidth(percent - self.delta))
		if start > 0:
			changeWidth(1)

	def slideRight(self, time = None, initialize = False):
		'''Slides a widget right (optional time parameter)
		...
		if end == 1:
			try:
				end = self.widget.previous_width
			except AttributeError:
				raise ValueError("Cannot calculate new height")
		def changeWidth(percent):
			self.widget.configure(height = end * percent)
...
class Tk(BaseWindow, TkBase):
	...
	This mainly includes support for the center and resize methods'''
